...
const IndexScreen = ({ navigation }) => {
  ...
  // Filter the posts by search query, change name to lowercase for better search
  const searchedRecipes = filteredState.filter(post =>
  ...
  const favoriteRecipes = filteredState.filter(post => post.favorite);
  const otherRecipes = filteredState.filter(post => !post.favorite);
